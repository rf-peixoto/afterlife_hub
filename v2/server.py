...
TLS_CERT_PATH = Path(os.environ.get("AFTERLIFE_TLS_CERT_PATH", "./data/certs/server.crt"))
TLS_KEY_PATH = Path(os.environ.get("AFTERLIFE_TLS_KEY_PATH", "./data/certs/server.key"))
TLS_MIN_VERSION = os.environ.get("AFTERLIFE_TLS_MIN_VERSION", "TLSv1_2")
...
def resolve_tls_min_version(name: str) -> ssl.TLSVersion:
...
def build_server_ssl_context(cert_path: Path, key_path: Path) -> ssl.SSLContext:
    ...
    ctx.minimum_version = resolve_tls_min_version(TLS_MIN_VERSION)
...
class Server:
    ...
    def serve(self) -> None:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
            ...
            self.sock = sock
            log(f"AFTERLIFE Space server listening with TLS on {self.host}:{self.port}")
            while not self.stop_event.is_set():
                try:
                    raw_client, addr = sock.accept()
                except OSError:
                    break

                try:
                    client = self.ssl_context.wrap_socket(raw_client, server_side=True)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="AFTERLIFE Space server – TLS socket server."
    )
    ...
    parser.add_argument("--cert", default=str(TLS_CERT_PATH), help=f"TLS certificate path (default: {TLS_CERT_PATH})")
    parser.add_argument("--key", default=str(TLS_KEY_PATH), help=f"TLS private key path (default: {TLS_KEY_PATH})")
...
if __name__ == "__main__":
    ...
    try:
        ssl_context = build_server_ssl_context(Path(args.cert), Path(args.key))
    except Exception as exc:
        log(f"Unable to initialize TLS context: {exc}")
        raise SystemExit(1)
